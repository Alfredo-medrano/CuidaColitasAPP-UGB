...
export default function NuevaVisita({ route, navigation }) {
  // ✅ defensivo: evita crash si params no existen
  const { petId = null, petName = 'Paciente no seleccionado' } = route?.params ?? {};
  ...
  if (loading) {
    return (
      <View style={styles.container}>
        <ActivityIndicator size="large" color="#013847" />
      </View>
    );
  }
