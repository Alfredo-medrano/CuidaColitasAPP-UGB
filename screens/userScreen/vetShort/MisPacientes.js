  ...
  const fetchPets = useCallback(async () => {
    try {
      ...
      if (role === 'veterinario') {
        ...
        if (searchTerm) {
          query = query.or(`name.ilike.%${searchTerm}%,owner.full_name.ilike.%${searchTerm}%`);
        }
      } else { // Cliente
      ...
      <View style={styles.card}>
        ...
        <View style={styles.cardActions}>
          ...
          <View style={styles.actionButtons}>
            <Pressable style={styles.actionButton} onPress={() => navigation.navigate('DetallePaciente', { petId: item.id, mode: 'view' })}>
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#43C0AF', paddingBottom: 15, paddingHorizontal: 15, paddingTop: 15 },
  ...
  statusBadge: { backgroundColor: '#e0ffe0', borderRadius: 15, paddingHorizontal: 10, paddingVertical: 5, marginLeft: 10 },
